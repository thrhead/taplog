...
import io.github.thrhead.taplog.core.domain.isValidFor
...
internal object PersistenceMapper {
    fun toRows(state: DomainState): PersistenceRows {
        ...
        requireInvariant(
            state.relationships.all { (key, value) -> key == (value.recordId to value.targetId) },
            "Relationship map key disagrees with IDs"
        )
        requireInvariant(
            state.stateGroups.all { (key, value) -> key == value.id },
            "State Group map key disagrees with ID"
        )
        requireInvariant(
            state.bindings.all { (key, value) -> key == value.bindingId },
            "Binding map key disagrees with ID"
        )
        requireInvariant(
            state.undoReceipts.all { (key, value) -> key == value.receiptId },
            "Undo map key disagrees with ID"
        )
        val rows = PersistenceRows(
            records = state.records.values.map {
                RecordEntity(
                    it.id.value, it.name, it.icon, encodeEnum(it.behavior),
                    encodeEnum(
                        it.lifecycle
                    ),
                    it.unit?.value, it.defaultQuantity?.let(::encodeQuantity), it.stateGroupId?.value,
                    it.revision.value, it.hasEvents
                )
            },
            targets = state.targets.values.map {
                TargetEntity(it.id.value, it.name, it.icon, encodeEnum(it.lifecycle), it.revision.value)
            },
            recordTargets = state.relationships.values.map {
                RecordTargetEntity(it.recordId.value, it.targetId.value, it.linked, it.revision.value)
            },
            ...
            stateScopes = (state.stateGenerations.keys + state.currentStates.keys).map { scope ->
                StateScopeEntity(
                    scope.groupId.value,
                    scopeKey(scope.targetId),
                    state.stateGenerations[scope]?.value ?: 0,
            ...
            bindings = state.bindings.values.map { binding ->
                ...
                BindingEntity(
                    binding.bindingId.value, binding.recordId.value, binding.targetId?.value,
                    encodeEnum(
                        binding.status
                    ),
            ...
            bindingUndoInvalidations = state.bindings.values.flatMap { binding ->
                binding.undoInvalidations.map {
            ...
            undoReceipts = state.undoReceipts.values.map { receipt ->
                UndoReceiptEntity(
                    ...
                    receipt.eventRevision.value,
                    receipt.datasetGeneration.value,
                    receipt.scope?.scope?.groupId?.value,
                    receipt.scope?.scope?.let {
                        scopeKey(it.targetId)
                    },
                    receipt.scope?.generation?.value
                )
            },
    ...
    fun fromRows(rows: PersistenceRows): DomainState {
        ...
        requireInvariant(
            metadata.singletonKey == 1 && metadata.schemaVersion == 1,
            "Unsupported Dataset metadata key or schema version"
        )
        val records = unique(
            rows.records.map { row ->
                Record(
                    RecordId(row.recordId), row.name, row.icon, enum(row.behavior),
                    enum(
                        row.lifecycle
                    ),
                    row.unit?.let(::unit),
                    row.defaultQuantity?.let(::decodeQuantity),
                    row.stateGroupId?.let(::StateGroupId),
                    Revision(row.revision), row.hasEvents
                )
            }
        ) { it.id }
        val targets = unique(
            rows.targets.map {
                Target(TargetId(it.targetId), it.name, it.icon, enum(it.lifecycle), Revision(it.revision))
            }
        ) { it.id }
        val relationships = unique(
            rows.recordTargets.map {
                RecordTarget(RecordId(it.recordId), TargetId(it.targetId), it.linked, Revision(it.revision))
            }
        ) {
            it.recordId to it.targetId
        }
        val groups = unique(
            rows.stateGroups.map {
                StateGroup(StateGroupId(it.stateGroupId), it.name, Revision(it.revision))
            }
        ) { it.id }
        val scopeRows = unique(
            rows.stateScopes
        ) { StateScope(StateGroupId(it.stateGroupId), targetId(it.targetScopeKey)) }
        scopeRows.values.forEach {
            requireInvariant(
                it.resetSequence == null && it.resetAt == null,
                "State reset metadata is not represented by DomainState"
            )
        }
        val invalidations = unique(rows.bindingUndoInvalidations) {
            it.bindingId to it.receiptId
        }.values.groupBy { it.bindingId }
        val bindings = unique(
            rows.bindings.map { row ->
                BindingLifecycle(
                    ...
                    DisplaySnapshot(
                        row.snapshotRecordName,
                        row.snapshotRecordIcon,
                        row.snapshotTargetName,
                        row.snapshotTargetIcon
                    ),
                    invalidations[row.bindingId].orEmpty().map {
                        UndoInvalidation(
        ...
        invalidations.keys.forEach {
            requireInvariant(BindingId(it) in bindings, "Undo invalidation references a missing Binding")
        }
        val receipts = unique(
            rows.undoReceipts.map { row ->
                ...
                requireInvariant(
                    (row.operation == null) == (row.beforeImageJson == null),
                    "Undo operation and before-image must be populated together"
                )
                requireInvariant(
                    row.operation == null || (row.operation.isNotBlank() && row.beforeImageJson!!.isNotBlank()),
                    "Undo operation and before-image must not be blank"
                )
                row.invalidationReason?.let { enum<ResultReason>(it) }
                val scopePresent = listOf(
                    row.stateGroupId != null,
                    row.targetScopeKey != null,
                    row.expectedScopeGeneration != null
                )
                requireInvariant(
                    scopePresent.all { it } || scopePresent.none { it },
                    "Undo State scope is partially populated"
                )
                UndoReceipt(
                    ReceiptId(row.receiptId),
                    EventId(row.eventId),
                    Revision(row.expectedEventRevision),
                    DatasetGeneration(row.expectedDatasetGeneration),
                    row.stateGroupId?.let {
                        ScopeContext(
                            StateScope(StateGroupId(it), targetId(row.targetScopeKey!!)),
                            DatasetGeneration(row.expectedScopeGeneration!!)
                        )
                    }
                )
            }
        ...
        val state = DomainState(
            records, targets, relationships, events, groups,
            DatasetGeneration(
                metadata.datasetGeneration
            ),
    ...
    private fun eventToRow(event: Event): EventEntity {
        ...
        return EventEntity(
            event.id.value, event.recordId.value, event.targetId?.value,
            scopeKey(
                event.targetId
            ),
            encodeEnum(event.behavior),
            event.occurredAt.value, event.createdAt.value, event.updatedAt.value, event.sequence.value,
            encodeEnum(
                event.source
            ),
            event.revision.value,
            snapshot.recordName, snapshot.recordIcon, snapshot.targetName, snapshot.targetIcon,
            encodeEnum(
                snapshot.behavior
            ),
            snapshot.unit?.value,
            counter?.quantity?.let(
                ::encodeQuantity
            ),
            counter?.unit?.value, duration?.startAt?.value, duration?.endAt?.value,
            duration?.status?.let(
                ::encodeEnum
            ),
            duration?.incompleteReason, state?.stateGroupId?.value, state?.generation?.value
        )
    }
    ...
    private fun eventFromRow(row: EventEntity): Event {
        ...
        val durationColumns = listOf(
            row.durationStartAt,
            row.durationEndAt,
            row.durationStatus,
            row.durationIncompleteReason
        )
        val stateColumns = listOf(row.stateGroupId, row.stateGeneration)
        requireInvariant(
            behavior == Behavior.COUNTER || counterColumns.all {
                it == null
            },
            "Extraneous Counter payload columns"
        )
        requireInvariant(
            behavior == Behavior.DURATION || durationColumns.all {
                it == null
            },
            "Extraneous Duration payload columns"
        )
        requireInvariant(
            behavior == Behavior.STATE || stateColumns.all { it == null },
            "Extraneous State payload columns"
        )
        val payload = when (behavior) {
            Behavior.MOMENT -> EventPayload.Moment
            Behavior.COUNTER -> EventPayload.Counter(
                decodeQuantity(required(row.counterQuantity, "Counter quantity")),
                row.counterUnit?.let(::unit)
            )
            Behavior.DURATION -> EventPayload.Duration(
                EpochMillis(required(row.durationStartAt, "Duration start")),
                row.durationEndAt?.let(::EpochMillis),
                enum(required(row.durationStatus, "Duration status")),
                row.durationIncompleteReason
            )
            Behavior.STATE -> EventPayload.State(
                StateGroupId(required(row.stateGroupId, "State group")),
                DatasetGeneration(required(row.stateGeneration, "State generation"))
            )
        }
        ...
        val snapshot = EventSnapshot(
            ...
            row.snapshotTargetName,
            row.snapshotTargetIcon,
            enum(row.snapshotBehavior),
            row.snapshotUnit?.let(::unit)
        )
        ...
        requireInvariant(
            payload !is EventPayload.Counter || snapshot.unit == payload.unit,
            "Counter unit disagrees with snapshot"
        )
        return Event(
            EventId(
                row.eventId
            ),
            RecordId(row.recordId), target, behavior, EpochMillis(row.occurredAt), EpochMillis(row.createdAt),
            EpochMillis(
                row.updatedAt
            ),
            Sequence(row.sequence), enum(row.source), Revision(row.revision), snapshot, payload
        )
    }

    // This is an exhaustive aggregate-invariant pass; every branch owns a distinct
    // persistence contract, so splitting it would hide rather than reduce complexity.
    ...
    private fun validate(state: DomainState) {
        ...
        state.records.values.forEach { record ->
            ...
            record.stateGroupId?.let {
                requireInvariant(
                    it in state.stateGroups,
                    "Record references a missing State Group"
                )
            }
            if (record.behavior == Behavior.STATE) {
                requireInvariant(
                    record.stateGroupId != null,
                    "State Record requires a State Group"
                )
            }
            if (record.behavior == Behavior.COUNTER) {
                requireInvariant(
                    record.defaultQuantity?.isValid == true,
                    "Counter default quantity must be positive"
                )
            }
        }
        ...
        state.events.forEach { event ->
            ...
            requireInvariant(
                event.sequence.value > 0 && event.sequence.value < state.nextSequence.value,
                "Event sequence outside allocation boundary"
            )
            ...
            if (payload is EventPayload.Duration && payload.status == DurationStatus.OPEN) {
                requireInvariant(
                    openScopes.add(event.recordId to event.targetId),
                    "Multiple OPEN Durations in one scope"
                )
            }
            if (payload is EventPayload.State) {
                ...
                requireInvariant(
                    state.records.getValue(event.recordId).stateGroupId == payload.stateGroupId,
                    "Event State Group disagrees with Record"
                )
        ...
        state.currentStates.forEach { (scope, recordId) ->
            ...
            state.records[recordId]?.let { record ->
                requireInvariant(
                    record.behavior == Behavior.STATE && record.stateGroupId == scope.groupId,
    ...
    private fun scopeKey(targetId: TargetId?): String = encodeTargetScope(
    ...
    private fun unit(text: String): UnitName = UnitName(text).also {
        requireInvariant(it.isValid, "Unit must not be blank")
    }
