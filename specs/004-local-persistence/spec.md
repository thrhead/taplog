...
4. **Given** Target permanent delete, **When** işlem tamamlanır, **Then** yalnız o Target'a ait Event/ilişki/binding/Undo metadata kaldırılır; diğer Target ve no-Target geçmişi korunur.
...
1. **Given** beklenen revision veya generation güncel değil, **When** Engine commit dener, **Then** veri katmanı yeni durumu korur ve `Conflict` sonucunun dönmesine izin verir.
...
3. **Given** receipt tüketilmiş, stale veya archive/unlink ile geçersizleşmiş, **When** yeniden kullanılır, **Then** değişiklik yapılmaz ve deterministic conflict sonucu korunur.
...
- Permanent delete kapsamı commit öncesi değişmişse, eski confirmation genişletilmiş kapsam için yeterli sayılmaz; commit reddedilir veya yeniden confirmation istenir.
...
- **FR-019**: Local storage, Event history ve gerekli metadata için güvenilir kronolojik sorgu, Record/Target kapsamı sorgusu ve current Duration/State kapsamı sorgusu sağlamalı; bu dilimde Timeline UI veya statistics hesaplama sunmamalıdır. (PRD §§12, 33; kapsam dışları.)
...
- **SC-007**: Core test ortamı, Android framework, UI veya ağ olmadan mapping, repository contract, atomicity, rollback ve deterministic failure testlerini çalıştırabilir; core modülünde veri teknolojisi bağımlılığı bulunmaz.
